...
const optimizeBuild = (props) => {
  var results = [];
  var inventorySize = props.possibleEngravings.length + props.lockedEngravings.length;
  var buildLimit = (inventorySize < props.buildLimit) ? inventorySize : props.buildLimit;
  var sizeDesired = buildLimit - props.lockedEngravings.length;

  // get all the build combinations we want out of the possible engravings chosen
  var combinations = powerSet(props.possibleEngravings, sizeDesired);
